...
Функция РазобратьСтруктуру(ЧтениеXML, Виды, Вид, ЧитатьВСоответствие)
	...
	Пока ЧтениеXML.ReadAttribute() Цикл
		...
		Если ВидАтрибута <> Неопределено Тогда
			Объект.Вставить(ИмяАтрибута, ВидАтрибута.ПривестиЗначение(ЧтениеXML.Значение));
		КонецЕсли;
	КонецЦикла;
	...
	Если ЧтениеXML.ТипУзла = ТипУзлаXML.Текст Тогда
		...
		ВидСвойства = Вид[ИмяСвойства];
		Если ВидСвойства <> Неопределено Тогда
			Объект.Вставить(ИмяСвойства, ВидСвойства.ПривестиЗначение(ЧтениеXML.Значение));
		КонецЕсли;
		ЧтениеXML.Прочитать();	 // node end
	КонецЕсли;
	Возврат Объект;
...
Функция Виды() Экспорт
	...
	// basic
	Виды.Вставить("String", Новый ОписаниеТипов("Строка"));
	Виды.Вставить("Boolean", Новый ОписаниеТипов("Булево"));
	Виды.Вставить("Decimal", Новый ОписаниеТипов("Число"));
...
Процедура ЗаменитьСсылкиНаТипы(Виды, Объект)
	Ключи = Новый Массив;
	...
	Для Каждого Ключ Из Ключи Цикл
		...
		Если ТипЗнч(Значение) = Тип("Строка") Тогда
			Объект[Ключ] = Виды[Значение];
		ИначеЕсли ТипЗнч(Значение) = Тип("Соответствие")
			Or ТипЗнч(Значение) = Тип("Структура") Тогда
			ЗаменитьСсылкиНаТипы(Виды, Значение);
		КонецЕсли;
	КонецЦикла;
